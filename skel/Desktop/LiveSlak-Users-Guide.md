...
- Riot-web   
	...
	（它需要通过浏览器来使用。默认是`firefox`，也可以从别的浏览器里访问：`http://localhost:8000` 即可使用；也可修改`/usr/local/bin/riot-web` 脚本；如你的 Matrix 帐号注册地不是官方的服务器，需要在`/etc/riot-web/config.json`里设置。）
- uTox   
	（p2p架构的（去中心化）通讯工具）   
- MatterMost
	（类似 Slack 的开源替代）
